    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Plasma(float2 uv)
            {
                ...
                float N2 = sin(SubFinalWave) * noise / uv.y;
                float FinalWave = N2 + SubFinalWave;
                return float3 (sin(FinalWave),1,1);
             
            }
             float3 PlasmaLarger(float2 uv)
            {
                 float time = _Time.y * Time;
                uv = (uv + .6) * _Scale;
                float w1 = sin((uv.x / 2) + (1.4* time));
                ...
                float SubFinalWave = (w1 + w2 + w3 + noise);
                float N2 = sin(SubFinalWave) * noise / uv.y;
                float FinalWave = sin(N2 + SubFinalWave);
                return float3 (-(FinalWave),-FinalWave,-FinalWave);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed3 plasmalarger = PlasmaLarger((i.uv));
                float3 FinalCol = plasmalarger % plasma + float3 (frac(_Time.y * i.uv.x % i.uv.y),.5,.5) +
                    float3 (0,0,(frac(i.uv.x) * .01));
